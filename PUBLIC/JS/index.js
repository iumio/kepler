...
$(document).ready(function () {
    ...
    $(".form_add_table").each(function () {
        $(this).submit(function (e) {
            ...
            var name_db = $("input[name='db_name']").val();
            var field_name = $('input[name^=field_name]').map(function () {
                return $(this).val();
            }).get();
            console.log(field_name);
            var field_type = $('select[name^=field_type]').map(function () {
                return $(this).val();
            }).get();
            console.log(field_type);
            var field_size = $('input[name^=field_size]').map(function () {
                return $(this).val();
            }).get();
            console.log(field_size);
            var select_default = $('select[name^=select_default]').map(function () {
                return $(this).val();
            }).get();
            console.log(select_default);
            var is_null = $('select[name^=is_null]').map(function () {
                return $(this).val();
            }).get();
            console.log(is_null);
            var select_index = $('select[name^=select_index]').map(function () {
                return $(this).val();
            }).get();
            console.log(select_index);
            var is_ai = $('select[name^=is_ai]').map(function () {
                return $(this).val();
            }).get();
            console.log(is_ai);
            var default_i = $('input[name^="default"]').map(function () {
                return $(this).val();
            }).get();
    ...
    $(".form_delete_field").each(function () {
        $(this).submit(function (e) {
            ...
            /****************************************
            MODEL FUNCTION
             public function drop_db($db, $table, $field_name)
             {
                 try {
                     $result = Connector::prepare("ALTER TABLE $db.$table DROP $field_name;", NULL);
                     return $result;
            ...
            rq.success(function (result) {
                ...
                if (result != 1) {
                    ...
                    $("#modal_info").modal("show");
                    window.setTimeout(function () {
                        window.location.href = 'index.php?run=content_table&dbname=' + db_name + '&t_name=' + table_name;
                    }, 2000);
                }
            })
        })
